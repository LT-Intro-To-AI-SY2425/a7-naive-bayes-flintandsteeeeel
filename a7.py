...
class BayesClassifier:
    ...
    def train(self) -> None:
        ...
        stopwords = self.tokenize(self.load_file("sorted_stoplist.txt"))
        for index, filename in enumerate(files, 1):
            print(f"Training {index/len(files)*100:.2f}%")
            tokens = self.tokenize(self.load_file(os.path.join(self.training_data_directory, filename)))
    ...
    def classify(self, text: str) -> str:
        ...
        # individual feature
        pos_denom = sum(self.pos_freqs.values())
        neg_denom = sum(self.neg_freqs.values())
        ...
        # of each probability for add one smoothing (so that we never have a probability of 0)
        for token in tokens:
            if token not in stopwords:
                pos_freqs = self.pos_freqs.get(token, 0) + 1
                neg_freqs = self.neg_freqs.get(token, 0) + 1

                pos_score += math.log(pos_freqs / (pos_denom + len(vocab)))
                neg_score += math.log(neg_freqs / (neg_denom + len(vocab)))
...
if __name__ == "__main__":
    ...
    assert b.classify('rainy days are the worst') == "negative", "classify test 3"
    print(b.classify('computer science is terrible'))
    assert b.classify('computer science is terrible') == "negative", "classify test 4"
